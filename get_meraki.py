...
Purpose: This script uses Meraki API to fetch list of organisations, networks and devices, it leverages Cisco DevNet sandboxes.
"""
...
class Meraki:
    ...
    def http_get_requests(self, url, headers, auth_username=None, auth_password=None):
        """
        ...
        :param auth_username: Username for basic authenctication if required else None will be used.
        :param auth_password: Password for basic authenctication if required else None will be used.
    ...
    def get_devices_in_organisations(self):
        """
        This fucntion fetches list of organisations, networks inside the oraganisation and devices inside the network.
        :return: Returns the list of devices if found else returns a empty list of JSON object.
        """
        devices = {}
        try:
            ...
            for organisation in organisations:
                if 'devnet' in organisation['name'].lower():
                    ...
                    networks = self.http_get_requests(f"{self.api_url}/organizations/{organisation_id}/networks", self.headers)
                    for network in networks:
                        network_id = network['id']
                        logging.info(f"Network IDs found in {organisation_id} are: {network_id}")
                        devices = self.http_get_requests(f"{self.api_url}/organizations/{organisation_id}/networks/{network_id}/devices", self.headers)
